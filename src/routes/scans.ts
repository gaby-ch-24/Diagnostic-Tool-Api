...
const routes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/scans', async (req, reply) => {
    ...
    await scansQueue.add('scan', {
      scanId: scan.id,
      url: scan.url,
      maxLinks: body.maxLinks ?? Number(process.env.SCAN_MAX_LINKS || 800),
      concurrency: body.concurrency ?? Number(process.env.SCAN_CONCURRENCY || 16)
  ...
  app.post('/scans/:id/retry', async (req, reply) => {
    ...
    if (!scan) return reply.code(404).send({ error: 'Not found' });
    await prisma.scan.update({ where: { id }, data: { status: 'QUEUED', startedAt: null, finishedAt: null, durationMs: null } });
    await scansQueue.add('scan', { scanId: id, url: scan.url, maxLinks: Number(process.env.SCAN_MAX_LINKS || 800), concurrency: Number(process.env.SCAN_CONCURRENCY || 16) }, { removeOnComplete: true, removeOnFail: true });
    reply.send({ ok: true });
  });
};
